...
with st.container():
    ...
    with col1:
        ...
        st.selectbox(label="角色画风", options=("超高清", "抽象", "水墨画", "二次元"), key="bot_style", on_change=lambda : st.session_state["meta"].update(bot_style=st.session_state["bot_style"]), help="角色的画风，默认二次元")
...
def draw_new_image():
    ...
    # TODO: 加上风格选项
    img_style = st.session_state["meta"].get("bot_style","二次元")
    image_prompt = img_style + "风格。" + image_prompt.strip()
